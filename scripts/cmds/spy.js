  ...
  config: {
   ...
   onStart: async function ({ event, message, usersData, api, args, getLang }) {
    ...
    api.getUserInfo(uid, async (err, userInfo) => {
      ...
      const userInformation = `❏ Name: ${userInfo[uid].name}\ Profile URL: ${userInfo[uid].profileUrl}\ Gender: ${genderText}\ User Type: ${userInfo[uid].type}\ Is Friend: ${userInfo[uid].isFriend ? "Yes" : "No"}\ Is Birthday today: ${userInfo[uid].isBirthday ? "Yes" : "No"}`;
